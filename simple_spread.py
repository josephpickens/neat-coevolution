...
class SimpleSpreadScenario(BaseScenario):
    ...
    def reward(self, agent, world):
        ...
        for l in world.landmarks:
            dists = [np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for a in world.agents]
            # dists = [np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for a in [agent]]
            rew -= min(dists)
        ...
        if agent.collide:
            ...
            for landmark in world.landmarks:
                if self.is_collision(agent, landmark):
                    ...
                    for a in world.agents:
                        if a != agent and self.is_collision(a, landmark):
    ...
    def observation(self, agent, world):
        ...
        # x-vel, y-vel, x-pos, y-pos, x-lm0-vec, y-lm1-vec, x-lm2-vec, y-lm2-vec, x-other-vec, y-other-vec, x-other-vel, y-other-vel
        return obs
